...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">  {{ __('laboratorie_employee.View_analysis') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{$rays->Patient->name}}</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
    ...
    <div class="form-group">
        <label for="exampleFormControlTextarea1">  {{ __('laboratorie_employee.Laboratory_Note') }}</label>
    ...
    <div class="demo-gallery">
        <ul id="lightgallery" class="list-unstyled row row-sm pr-0">
            ...
            <li class="col-sm-6 col-lg-4" data-responsive="{{URL::asset('Dashboard/img/laboratories/'.$image->filename)}}" data-src="{{URL::asset('Dashboard/img/laboratories/'.$image->filename)}}">
                <a href="#" onclick="openImageWindow('{{ asset('Dashboard/img/laboratories/' . $image->filename) }}')">
